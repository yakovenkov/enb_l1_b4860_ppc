 ...
 */
asn_dec_rval_t
SEQUENCE_decode_gser(const asn_codec_ctx_t *opt_codec_ctx,
                    const asn_TYPE_descriptor_t *td, void **struct_ptr,
                    const char *opt_mname, const void *ptr, size_t size) {
    /*
	 * Bring closer parts of structure description.
	...
	for (int i = 0; i <= td->elements_count;)
	{
		...
		{
			ADVANCE(token_size);
			token_size = gser_next_token(&ctx->context, ptr, size, &token);
		} while(token_size > 0 && token == GSER_TOKEN_SPACE);

		if (token_size <= 0)
		{
			if ((td->elements_count > i))
			{
				if(td->elements[i].optional)
				{
					token = GSER_TOKEN_TEXT;
				}
				else
				{
					if (gser_decoder_is_error_str_filled == 0)
					{
						gser_decoder_is_error_str_filled = 1;
						
						snprintf(gser_decoder_error_str,
							GSER_DECODER_ERROR_STR_LEN,
							"Absent mandatory field %s at line %i",
							td->elements[i].name,
							gser_get_line(gser_decoder_str, ptr));
		...
		if (token == GSER_TOKEN_OPEN_BRACKET)
		{
			XER_ADVANCE(token_size);
			num_braces++;
			continue;
		}
		else if (token == GSER_TOKEN_COMMA)
		{
			XER_ADVANCE(token_size);
			continue;
		}
		else if (token == GSER_TOKEN_CLOSE_BRACKET)
		{
			if ((i < td->elements_count)&&(td->elements[i].optional))
			{
				token = GSER_TOKEN_TEXT;
			}
			else if (i == td->elements_count)
			{
				XER_ADVANCE(token_size);
				num_braces--;
				if (num_braces == 0)
		...
		else if (token != GSER_TOKEN_TEXT)
		{
			...
			if (gser_decoder_is_error_str_filled == 0)
			{
				...
				snprintf(gser_decoder_error_str,
				         GSER_DECODER_ERROR_STR_LEN,
				         "Unexpected token %s at line %i: %.*s...",
				         gser_get_token_name(token),
				         gser_get_line(gser_decoder_str, (char*)ptr),
				         16,
				         ptr);
		...
		if (token == GSER_TOKEN_TEXT)
		{
			...
			if (i >= td->elements_count)
			{
				...
				if (gser_decoder_is_error_str_filled == 0)
				{
					...
					snprintf(gser_decoder_error_str,
					         GSER_DECODER_ERROR_STR_LEN,
					         "Unknown sequence field at line %i: %.*s...",
					         gser_get_line(gser_decoder_str, (char*)ptr),
					         16,
					         ptr);
			...
			if ((token_size == strlen(elm->name) &&
				(memcmp(ptr, elm->name, token_size) == 0)))
			{
				ADVANCE(token_size);
				if(elm->flags & ATF_POINTER) {
					/* Member is a pointer to another structure */
				...
				if(elm->flags & ATF_OPEN_TYPE) {
					...
					/* Invoke the inner type decoder, m.b. multiple times */
					tmprval = elm->type->op->gser_decoder(opt_codec_ctx,
							elm->type, memb_ptr2, elm->name,
							ptr, size);
				}
				if (tmprval.code == RC_OK || tmprval.code == RC_EMPTY)
				{
					i++;
					num_items++;
					XER_ADVANCE(tmprval.consumed);
			...
			else
			{
				...
				{
					//skip optional fieldif(elm->default_value_set) {
					if (elm->default_value_set) {
                        if(elm->flags & ATF_POINTER) {
							/* Member is a pointer to another structure */
							memb_ptr2 = (void **)((char *)st + elm->memb_offset);
					...
					//default_value_set(elm->default_value_set);
					i++;
				}
			}
		}
		...
		if ((num_braces == 0)&&(i == td->elements_count))
			break;
...
asn_enc_rval_t
SEQUENCE_encode_gser(const asn_TYPE_descriptor_t *td, const void *sptr,
                    int ilevel, enum gser_encoder_flags_e flags,
                    asn_app_consume_bytes_f *cb, void *app_key) {
    asn_enc_rval_t er;
    asn_TYPE_descriptor_t *tmp_def_val_td = 0;
    void *tmp_def_val = 0;
    ...
    for(edx = 0; edx < td->elements_count; edx++) {
		...
		if (edx - num_optional > 0)
            ASN__CALLBACK(",\n", 2);

        ASN__TEXT_INDENT(0, ilevel);
		ASN__CALLBACK2(mname, mlen, " ", 1);

        /* Print the member itself */
        tmper = elm->type->op->gser_encoder(elm->type, memb_ptr, ilevel + 1,
                                           flags, cb, app_key);
...
asn_TYPE_operation_t asn_OP_SEQUENCE = {
	...
	SEQUENCE_decode_xer,
	SEQUENCE_encode_xer,
	SEQUENCE_decode_gser,
	SEQUENCE_encode_gser,
