...
asn_TYPE_operation_t asn_OP_NativeInteger = {
	...
	NativeInteger_decode_gser,
	NativeInteger_encode_gser,
 ...
 */
asn_dec_rval_t
NativeInteger_decode_gser(const asn_codec_ctx_t *opt_codec_ctx,
                         const asn_TYPE_descriptor_t *td, void **sptr,
                         const char *opt_mname, const void *buf_ptr,
                         size_t size) {
    const asn_INTEGER_specifics_t *specs =
	...
	memset(&st, 0, sizeof(st));
	rval = INTEGER_decode_gser(opt_codec_ctx, td, &st_ptr, opt_mname, buf_ptr, size);
	if(rval.code == RC_OK) {
		long l = 0;
		if((specs&&specs->field_unsigned)
			? asn_INTEGER2ulong(&st, (unsigned long *)&l) /* sic */
...
asn_enc_rval_t
NativeInteger_encode_gser(const asn_TYPE_descriptor_t *td, const void *sptr,
