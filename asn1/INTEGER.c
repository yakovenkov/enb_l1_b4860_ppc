...
INTEGER_gser_map_enum2value(const asn_INTEGER_specifics_t *specs, const char *lstart,
                       const char *lstop) {
...
INTEGER__gser_body_decode(const asn_TYPE_descriptor_t *td, void *sptr,
                         const void *chunk_buf, size_t chunk_size) {
    const asn_INTEGER_specifics_t *specs =
        (const asn_INTEGER_specifics_t *)td->specifics;
    INTEGER_t *st = (INTEGER_t *)sptr;
	...
	el = INTEGER_gser_map_enum2value(
		(const asn_INTEGER_specifics_t *)
		td->specifics, lstart, lstop);
	...
	//Не удалось найти подходящее значение enum
	if ((td->specifics) &&
	    (((const asn_INTEGER_specifics_t *)td->specifics)->value2enum) &&
	    (!el))
	{
		return GPBD_SYSTEM_FAILURE;
	}
...
asn_dec_rval_t
INTEGER_decode_gser(const asn_codec_ctx_t *opt_codec_ctx,
                   const asn_TYPE_descriptor_t *td, void **sptr,
                   const char *opt_mname, const void *buf_ptr, size_t size) {
    ...
    asn_dec_rval_t rv =  gser_decode_primitive(opt_codec_ctx, td,
		                                       sptr, sizeof(INTEGER_t), opt_mname,
		                                       buf_ptr, size, INTEGER__gser_body_decode);
...
asn_enc_rval_t
INTEGER_encode_gser(const asn_TYPE_descriptor_t *td, const void *sptr,
