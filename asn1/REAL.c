...
ssize_t
REAL__dump(double d, int canonical, asn_app_consume_bytes_f *cb, void *app_key, int plainOrXER) {
	char local_buf[64];
	char *buf = local_buf;
...
REAL_print(const asn_TYPE_descriptor_t *td, const void *sptr, int ilevel,
	...
	if(!st || !st->buf)
		...
		ret = REAL__dump(d, 0, cb, app_key, 1);
...
REAL_encode_xer(const asn_TYPE_descriptor_t *td, const void *sptr, int ilevel,
	...
	er.encoded = REAL__dump(d, flags & XER_F_CANONICAL, cb, app_key, 1);
...
/*
 * Decode the chunk of GSER text encoding REAL.
 */
static enum gser_pbd_rval
REAL__gser_body_decode(const asn_TYPE_descriptor_t *td, void *sptr,
                      const void *chunk_buf, size_t chunk_size) {
    REAL_t *st = (REAL_t *)sptr;
	double value;
	const char *xerdata = (const char *)chunk_buf;
	...
	if(!chunk_size) return GPBD_BROKEN_ENCODING;

	/*
	 * Decode an XMLSpecialRealValue: <MINUS-INFINITY>, etc.
	 */
	if(xerdata[0] == 'i' || xerdata[0] == 'n')
	{
		double dv;
		if(strcmp(xerdata, "inf+") == 0)
			dv = INFINITY;
		else if(strcmp(xerdata, "inf-") == 0)
			dv = -INFINITY;
		else if(strcmp(xerdata, "nan") == 0)
			dv = NAN;
		else
			return GPBD_SYSTEM_FAILURE;

		if(asn_double2REAL(st, dv))
			return GPBD_SYSTEM_FAILURE;

		return GPBD_BODY_CONSUMED;
	}
	 ...
	 */
	b = (char *)MALLOC(chunk_size + 1);
	if(!b) return GPBD_SYSTEM_FAILURE;
	memcpy(b, chunk_buf, chunk_size);
	b[chunk_size] = 0;	/* nul-terminate */

	value = strtod(b, &endptr);
	FREEMEM(b);
	if(endptr == b) return GPBD_BROKEN_ENCODING;
...
asn_dec_rval_t
REAL_decode_gser(const asn_codec_ctx_t *opt_codec_ctx,
                   const asn_TYPE_descriptor_t *td, void **sptr,
                   const char *opt_mname, const void *buf_ptr, size_t size) {
                   
    GSER_PRINTF("<int %s>\n", td->xml_tag);
                   
    asn_dec_rval_t rv =  gser_decode_primitive(opt_codec_ctx, td,
		                                       sptr, sizeof(REAL_t), opt_mname,
		                                       buf_ptr, size, REAL__gser_body_decode);
	...
	if (rv.code != RC_OK)
	{
		if (gser_decoder_is_error_str_filled == 0)
		{
			...
			snprintf(gser_decoder_error_str,
			         ...
			         "Invalid value for %s at line %i: %.*s...",
			         td->xml_tag,
			         gser_get_line(gser_decoder_str, (char*)buf_ptr),
			         16,
			         buf_ptr);
...
asn_enc_rval_t
REAL_encode_gser(const asn_TYPE_descriptor_t *td, const void *sptr,
	...
	er.encoded = REAL__dump(d, flags & GSER_F_CANONICAL, cb, app_key, 1);
	if(er.encoded < 0) ASN__ENCODE_FAILED;
