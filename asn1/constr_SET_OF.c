 ...
 */
asn_dec_rval_t
SET_OF_decode_gser(const asn_codec_ctx_t *opt_codec_ctx,
                  const asn_TYPE_descriptor_t *td, void **struct_ptr,
                  const char *opt_mname, const void *buf_ptr, size_t size) {
    /*
	 * Bring closer parts of structure description.
	...
	for(;;)
	{
		...
		if (token == GSER_TOKEN_OPEN_BRACKET)
		{
			XER_ADVANCE(token_size);
			num_braces++;
			//Открывающаяся скобка, дальше нужно пробовать разбирать вложенный тип
		}
		else if (token == GSER_TOKEN_COMMA)
		{
			XER_ADVANCE(token_size);
			//запятая, дальше нужно пробовать разбирать след. элемент
			//continue;
		}
		...
		/* Invoke the inner type decoder, m.b. multiple times */
		ASN_DEBUG("XER/SET OF element [%s]", elm_tag);
		tmprval = element->type->op->gser_decoder(opt_codec_ctx,
				element->type, &ctx->ptr, elm_tag,
				buf_ptr, size);
		if(tmprval.code == RC_OK) {
			num_items++;
...
asn_enc_rval_t
SET_OF_encode_gser(const asn_TYPE_descriptor_t *td, const void *sptr, int ilevel,
