...
BOOLEAN__gser_body_decode(const asn_TYPE_descriptor_t *td, void *sptr,
                         const void *chunk_buf, size_t chunk_size) {
    BOOLEAN_t *st = (BOOLEAN_t *)sptr;
	const char *p = (const char *)chunk_buf;

	static const char true_lexem[] = "TRUE";
	static const char false_lexem[] = "FALSE";

	(void)td;

	if ((chunk_size == strlen(true_lexem)) &&
	    (strncasecmp(p, true_lexem, strlen(true_lexem)) == 0))
	{
		*st = 1;
		return XPBD_BODY_CONSUMED;
	}
	else if ((chunk_size == strlen(false_lexem)) &&
	         (strncasecmp(p, false_lexem, strlen(false_lexem))) == 0)
	{
		*st = 0;
		return XPBD_BODY_CONSUMED;
	}
...
asn_dec_rval_t
BOOLEAN_decode_gser(const asn_codec_ctx_t *opt_codec_ctx,
                   const asn_TYPE_descriptor_t *td, void **sptr,
                   const char *opt_mname, const void *buf_ptr, size_t size) {
    GSER_PRINTF("<bool %s>\n", td->xml_tag);
    return gser_decode_primitive(opt_codec_ctx, td,
		sptr, sizeof(BOOLEAN_t), opt_mname, buf_ptr, size,
		BOOLEAN__gser_body_decode);
}

asn_enc_rval_t
BOOLEAN_encode_gser(const asn_TYPE_descriptor_t *td, const void *sptr,
