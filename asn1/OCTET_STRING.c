...
asn_TYPE_operation_t asn_OP_OCTET_STRING = {
	...
	OCTET_STRING_decode_gser_hex,
	OCTET_STRING_encode_gser,
...
/*
 * Convert from binary format: '00101011101'B
 */
static ssize_t OCTET_STRING__gser_convert_binary(void *sptr, const void *chunk_buf, size_t chunk_size, int have_more) {
	BIT_STRING_t *st = (BIT_STRING_t *)sptr;
	
	if (chunk_size < 3)
		return -1;

	size_t string_size = chunk_size - 3;
	...
	/* Reallocate buffer according to high cap estimation */
	size_t new_size = st->size + (string_size + 7) / 8;
	void *nptr = REALLOC(st->buf, new_size + 1);
	if(!nptr) return -1;
	st->buf = (uint8_t *)nptr;
	buf = st->buf + st->size;
	...
	for(; p < pend; p++) {
		...
		switch(ch) {
		case 0x09: case 0x0a: case 0x0c: case 0x0d:
		case 0x27: case 0x42:	//single quote, 'B'
		case 0x20:
			/* Ignore whitespace */
			break;
		case 0x30:
...
static ssize_t OCTET_STRING__gser_convert(void *sptr, const void *chunk_buf, size_t chunk_size, int have_more)
{
	int is_hex = 0;
	//Определение Bin формат или Hex
	char* format_specifier = (char*)chunk_buf + chunk_size;
	while(format_specifier != chunk_buf)
	{
	
		if (*format_specifier == 'B' || (*format_specifier == 'b'))
		{
			is_hex = 0;
			break;
		}
		
		if (*format_specifier == 'H' || (*format_specifier == 'h'))
		{
			is_hex = 1;
			break;
		}
	...
	if (is_hex)
		...
		return OCTET_STRING__gser_convert_binary(sptr, chunk_buf, chunk_size, have_more);	
}
...
OCTET_STRING__convert_entrefs(void *sptr, const void *chunk_buf,
	...
	int quoter = *(const unsigned char *)p;
	p++;
	for(; p < pend; p++) {
		int ch = *(const unsigned char *)p;
...
OCTET_STRING__decode_gser(
	...
	return gser_decode_general(opt_codec_ctx, ctx, *sptr, xml_tag,
		buf_ptr, size, opt_unexpected_tag_decoder, body_receiver);
 ...
 */
asn_dec_rval_t
OCTET_STRING_decode_gser_hex(const asn_codec_ctx_t *opt_codec_ctx,
                            const asn_TYPE_descriptor_t *td, void **sptr,
                            const char *opt_mname, const void *buf_ptr,
                            size_t size) {
    return OCTET_STRING__decode_gser(opt_codec_ctx, td, sptr, opt_mname,
		buf_ptr, size, 0, OCTET_STRING__gser_convert);
}
 ...
 */
asn_dec_rval_t
OCTET_STRING_decode_gser_binary(const asn_codec_ctx_t *opt_codec_ctx,
                               const asn_TYPE_descriptor_t *td, void **sptr,
                               const char *opt_mname, const void *buf_ptr,
                               size_t size) {
    return OCTET_STRING__decode_gser(opt_codec_ctx, td, sptr, opt_mname,
 ...
 */
asn_dec_rval_t
OCTET_STRING_decode_gser_utf8(const asn_codec_ctx_t *opt_codec_ctx,
                             const asn_TYPE_descriptor_t *td, void **sptr,
                             const char *opt_mname, const void *buf_ptr,
                             size_t size) {
    return OCTET_STRING__decode_gser(opt_codec_ctx, td, sptr, opt_mname,
...
asn_enc_rval_t
OCTET_STRING_encode_gser(const asn_TYPE_descriptor_t *td, const void *sptr,
