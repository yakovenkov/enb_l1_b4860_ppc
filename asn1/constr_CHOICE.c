 ...
 */
asn_dec_rval_t
CHOICE_decode_gser(const asn_codec_ctx_t *opt_codec_ctx,
                  const asn_TYPE_descriptor_t *td, void **struct_ptr,
                  const char *opt_mname, const void *buf_ptr, size_t size) {
    /*
	 * Bring closer parts of structure description.
	...
	{
		const char* choice_str = buf_ptr;
		size_t choice_str_len = token_size;
		XER_ADVANCE(token_size);
		...
		if (token == GSER_TOKEN_COLON)
		{
			...
			for (int i = 0; i < td->elements_count; i++)
			{
				...
				if ((choice_str_len == strlen(td->elements[i].name) &&
					(memcmp(choice_str, td->elements[i].name, choice_str_len) == 0)))
				{
					if(elm->flags & ATF_POINTER) {
						/* Member is a pointer to another structure */
						memb_ptr2 = (void **)((char *)st
					...
					old_present = _fetch_present_idx(st,
						specs->pres_offset, specs->pres_size);
					assert(old_present == 0 || old_present == i + 1);
					/* Record what we've got */
					_set_present_idx(st,
						specs->pres_offset, specs->pres_size, i + 1);
		...
		{
			int offset = 0;
			offset += snprintf(gser_decoder_error_str + offset,
			                   GSER_DECODER_ERROR_STR_LEN - offset,
			                   "Invalid choice value at line %i %.*s",
			                   gser_get_line(gser_decoder_str, buf_ptr),
			                   choice_str_len,
			                   choice_str);
			
			if (td->elements_count > 0)
			{
				snprintf(gser_decoder_error_str + offset,
				         GSER_DECODER_ERROR_STR_LEN - offset,
				         " expect [%s...]",
				         td->elements[0].name);
			};
...
asn_enc_rval_t
CHOICE_encode_gser(const asn_TYPE_descriptor_t *td, const void *sptr, int ilevel,
                  enum gser_encoder_flags_e flags, asn_app_consume_bytes_f *cb,
                  void *app_key) {
    const asn_CHOICE_specifics_t *specs =
        (const asn_CHOICE_specifics_t *)td->specifics;
    asn_enc_rval_t er;
	...
	if(present == 0 || present > td->elements_count) {
		...
		ASN__CALLBACK2(mname, mlen, " : ", 3);

		tmper = elm->type->op->gser_encoder(elm->type, memb_ptr,
				ilevel, flags, cb, app_key);
		if(tmper.encoded == -1) return tmper;
		er.encoded += tmper.encoded;
	}
