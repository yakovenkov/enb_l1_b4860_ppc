...
asn_TYPE_operation_t asn_OP_NativeReal = {
	...
	NativeReal_decode_gser,
	NativeReal_encode_gser,
 ...
 */
asn_dec_rval_t
NativeReal_decode_gser(const asn_codec_ctx_t *opt_codec_ctx,
                   const asn_TYPE_descriptor_t *td, void **sptr,
                   const char *opt_mname, const void *buf_ptr, size_t size)
{
    asn_dec_rval_t rval;
	...
	rval = REAL_decode_gser(opt_codec_ctx, td, (void **)&stp, opt_mname,
		buf_ptr, size);
	if(rval.code == RC_OK) {
        double d;
        if(asn_REAL2double(&st, &d) || NativeReal__set(td, sptr, d) < 0) {
...
asn_enc_rval_t
NativeReal_encode_gser_(const asn_TYPE_descriptor_t *td, const void *sptr,
...
asn_enc_rval_t
NativeReal_encode_gser(const asn_TYPE_descriptor_t *td, const void *sptr,
                      int ilevel, enum gser_encoder_flags_e flags,
                      asn_app_consume_bytes_f *cb, void *app_key) {
    double d = NativeReal__get_double(td, sptr);
	asn_enc_rval_t er;
...
NativeReal_print(const asn_TYPE_descriptor_t *td, const void *sptr, int ilevel,
	...
	if(sptr) {
        double d = NativeReal__get_double(td, sptr);
        return (REAL__dump(d, 0, cb, app_key, 1) < 0) ? -1 : 0;
    } else {
